...
import MainPanel from './MainPanel'; // Renamed to QuotationPanel conceptually
...
const ERP: React.FC<ERPProps> = ({ user, onLogout }) => {
  ...
  const renderContent = () => {
    const panelWrapper = (content: React.ReactNode) => (
         <div className="bg-black/20 backdrop-blur-xl border border-gray-700/50 rounded-3xl shadow-2xl shadow-purple-500/10 p-6 md:p-8 transition-all duration-500 min-h-[calc(100vh-10rem)]">
            {content}
         </div>
    );

    if (isLoading) {
      return panelWrapper(<div className="text-center p-10 text-lg font-semibold text-gray-400">Cargando datos...</div>);
    }
    if (error) {
      return panelWrapper(<div className="text-center p-10 text-red-400">{error}</div>);
    }
