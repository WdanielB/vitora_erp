...
const ERP: React.FC<ERPProps> = ({ user: initialUser, onLogout }) => {
  ...
  const renderContent = () => {
    ...
    if (isLoading) {
      return panelWrapper(<div className="flex items-center justify-center h-full text-center p-10 text-lg font-semibold text-gray-400">Cargando datos...</div>, true);
    }
    if (error) {
      return panelWrapper(<div className="flex items-center justify-center h-full text-center p-10 text-red-400">{error}</div>, true);
    }
    
    switch(view) {
      case 'dashboard':
        return panelWrapper(<DashboardPanel orders={orders} financialSummary={financialSummary} stockItems={stock} />);
      case 'quotation':
        return panelWrapper(<MainPanel flowerItems={flowerItems} setFlowerItems={handleSetFlowerItems} fixedItems={fixedItems} />, true);
      case 'stock':
        return panelWrapper(<StockPanel stockItems={stock} onStockUpdate={() => loadData(selectedUserId)} user={user} selectedUserId={selectedUserId} flowerItems={flowerItems} setFlowerItems={handleSetFlowerItems} fixedItems={fixedItems} setFixedItems={handleSetFixedItems} />, true);
      case 'orders':
        return panelWrapper(<OrdersPanel orders={orders} allItems={[...flowerItems, ...fixedItems]} clients={clients} onDataNeedsRefresh={() => loadData(selectedUserId)} user={user} />);
      case 'calendar':
        return panelWrapper(<CalendarPanel events={events} orders={orders} user={user} onDataNeedsRefresh={() => loadData(selectedUserId)} />, true);
      case 'finance':
        return panelWrapper(<FinancePanel summary={financialSummary} fixedExpenses={fixedExpenses} orders={orders} user={user} onDataNeedsRefresh={() => loadData(selectedUserId)} flowerItems={flowerItems} setFlowerItems={handleSetFlowerItems} fixedItems={fixedItems} setFixedItems={handleSetFixedItems} />, true);
      case 'settings':
        return <SettingsPanel flowerItems={flowerItems} fixedItems={fixedItems} user={user} onUserPinsUpdate={onUserPinsUpdate} allUsers={allUsers} />;
      default:
        return panelWrapper(<DashboardPanel orders={orders} financialSummary={financialSummary} stockItems={stock} />);
    }
  }
